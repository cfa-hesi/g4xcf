...
void RunAction::EndOfRunAction(const G4Run*)
{
    ...
    if (detector_logic_vol)
    {
      ...
      G4Tubs * detector_solid_vol
        = dynamic_cast<G4Tubs*>(detector_logic_vol->GetSolid());

      double const detector_inner_radius = detector_solid_vol->GetInnerRadius() / CLHEP::cm;
      double const detector_outer_radius = detector_solid_vol->GetOuterRadius() / CLHEP::cm;
      double const detector_length_z = detector_solid_vol->GetZHalfLength() * 2./ CLHEP::cm;
      ...
      analysis_manager->FillMetadata(detector_inner_radius,
